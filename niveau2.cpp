...
if(touchH==0){
  for (brique = 0; brique < BLOCK_LENGTH+1; brique++) {
    mvaddch(H.y, H.x+brique, ' ');
    mvchgat(H.y, H.x, 11, ' ', BRED, NULL);
  }
}
else if(touchH==1){
  for (brique = 0; brique < BLOCK_LENGTH+1; brique++) {
    mvaddch(H.y, H.x+brique, ' ');
    mvchgat(H.y, H.x, 11, ' ', BGREEN, NULL);
  }
}
else if(touchH>1){
  for (brique = 0; brique < BLOCK_LENGTH+1; brique++) {
    ...
    H.x=0;
    H.y=0;
    if(touchH==2){nbBriques--; touchH++; score+=5;}
}
...
WINDOW *win;
    ...
    mvvline(0,SCREEN_WIDTH+1, ACS_VLINE,SCREEN_WIDTH+2);
...
else if((bally==G.y+1 && ballx>=G.x && ballx<=G.x+BLOCK_LENGTH) || (bally==G.y && ballx==G.x+BLOCK_LENGTH+1)) { //la balle touche la brique en bas ou à droite
  xMinus=false;
  yMinus=false;
        moveBallx(ballx, xMinus);
      moveBally(bally, yMinus);
...
else if((bally==G.y-1 && ballx>=G.x && ballx<=G.x+BLOCK_LENGTH) || (bally==G.y && ballx==G.x-1)){ //la balle touche la brique en haut ou à gauche
  xMinus=true;
  yMinus=true;
        moveBallx(ballx, xMinus);
      moveBally(bally, yMinus);
...
else if((bally==H.y+1 && ballx>=H.x && ballx<=H.x+BLOCK_LENGTH) || (bally==H.y && ballx==H.x+BLOCK_LENGTH+1)) { //la balle touche la brique en bas ou à droite
  xMinus=false;
  yMinus=false;
        moveBallx(ballx, xMinus);
      moveBally(bally, yMinus);
...
else if((bally==H.y-1 && ballx>=H.x && ballx<=H.x+BLOCK_LENGTH) || (bally==H.y && ballx==H.x-1)){ //la balle touche la brique en haut ou à gauche
  xMinus=true;
  yMinus=true;
        moveBallx(ballx, xMinus);
      moveBally(bally, yMinus);
